...
class SeasonData:
    ...
    def __init__(self, logger: logging.Logger) -> None:
        ...
        if not os.path.exists(file_path):
            ...
            self.logger.error(f"Line {current_line}: {error_msg}")
    ...
    def get_seasons(self, *, league_tier: Optional[int] = None) -> List[int]:
        ...
        if league_tier is not None:
            if not isinstance(league_tier, int) or league_tier < 1:
                error_msg = "League tier must be a positive integer"
                current_line = traceback.extract_stack()[-1].lineno
                self.logger.error(f"Line {current_line}: {error_msg}")
                raise ValueError(error_msg)
    ...
    def get_club_count(self, *, season: int, league_tier: int) -> int:
        ...
        if not isinstance(league_tier, int):
            error_msg = "League tier must be an integer"
            current_line = traceback.extract_stack()[-1].lineno
            self.logger.error(f"Line {current_line}: {error_msg}")
            raise ValueError(error_msg)
    ...
    def check_leagues_seasons(self, *, matches: pd.DataFrame, ignore_tiers: Optional[List[int]] = None) -> bool:
        """
        ...
        if ignore_tiers is None:
            ignore_tiers = []

        matches_summary = (
            matches[~matches['league_tier'].isin(ignore_tiers)][['season', 'league_tier']]
            .drop_duplicates()
        ...
        football_data_summary = (
            self.football_data[~self.football_data['league_tier'].isin(ignore_tiers)][['season', 'league_tier']]
            .drop_duplicates()
            .sort_values(by=['season', 'league_tier'])
            .reset_index(drop=True)
        )
    ...
    def check_leagues_seasons_clubs(self, *, matches: pd.DataFrame, ignore_tiers: Optional[List[int]] = None) -> bool:
        """
        Check that league seasons in matches dataframe match reference data.
        ...
        matches_summary = (
            matches[~matches['league_tier'].isin(ignore_tiers)][['season', 'league_tier', 'home_club']]
        ...
        football_data_summary = (
            self.football_data[~self.football_data['league_tier'].isin(ignore_tiers)][['season', 'league_tier', 'clubs']]
            .sort_values(by=['season', 'league_tier'])
        )
        ...
        if (not differences.empty or differences.shape[0] > 0):
    ...
    def check_leagues_seasons_matches(self, *, matches: pd.DataFrame, ignore_tiers: Optional[List[int]] = None) -> bool:
        """
        Check that league seasons in matches dataframe match reference data.
        ...
        football_data_summary = (
            self.football_data[~self.football_data['league_tier'].isin(ignore_tiers)][['season', 'league_tier', 'matches']]
            .sort_values(by=['season', 'league_tier'])
        )
        football_data_summary['league_tier'] = football_data_summary['league_tier'].astype(
            int
...
def check_clubs_1(*,
    ...
    if (matches['home_club'].isnull().any() or
        ...
        current_line = traceback.extract_stack()[-1].lineno
        logger.error(f"Line {current_line}: {error_msg}")
        if error_stop:
            raise ValueError(error_msg)
        return False
...
def check_clubs_2(*,
    ...
    if one_leg.shape[0] > 0:
        ...
        current_line = traceback.extract_stack()[-1].lineno
        logger.error(f"Line {current_line}: {error_msg_fmt}", str(one_leg.to_dict(orient='records')))
        if error_stop:
            raise ValueError(error_msg)
        return False
