...
def get_default_csv_path() -> Path:
    ...
    `3 Analysis/Team goal distribution/1_league_season_goals_clubs_binomial.py`
    The CSV lives in:
...
def compute_goal_distribution(
    ...
    goals_column: str,
) -> tuple[pd.Series, int, float, np.ndarray, float, float]:
    """
    Compute observed goal relative frequency distribution and Binomial fit
    ...
    distribution:
        ...
        (proportions).
    n:
        Number of trials for Binomial distribution (fixed at 10).
    p_hat:
        Estimated Binomial success probability.
    fitted_proportions:
        Expected relative frequencies for each goals value under the
        Binomial model, aligned to `distribution.index`.
    chi2_stat:
    ...
    # Fixed number of trials for Binomial distribution
    n = 10
    ...
    # Estimate Binomial success probability
    p_hat = mean_goals / float(n)

    # Compute fitted counts using Binomial distribution
    fitted_counts = (
        stats.binom.pmf(
            k=goals,
            n=n,
    ...
    return distribution, n, p_hat, fitted_proportions, chi2_stat, chi2_p_value
...
def plot_goal_distribution(
    *,
    ...
    distribution and optionally Binomial fitted proportions.
    ...
    fitted_proportions:
        Expected relative frequencies under Binomial model.
    chi2_stat:
        Chi-square goodness-of-fit statistic.
    chi2_p_value:
        p-value for the chi-square goodness-of-fit test.
    show_binomial:
        Whether to show the Binomial fit line.
...
if __name__ == "__main__":
    try:
        ...
        if len(valid_results) == 0:
            ...
            # Add Liverpool FC 2024-2025 charts
            liverpool_club_name = "Liverpool FC"
            liverpool_season = "2024-2025"
            
            # Find Liverpool FC's league_tier for 2024-2025
            ...
            if charts:
                ...
                output_file(
                    ...
                    title="Club Goal Distributions - Best/Worst Fit (Binomial)",
                )
