...
# Season start and end dates. Mostly estimated, but filled in by hand where needed
SEASON_START_END_MAP = pd.DataFrame([
    ...
    # Manually entered dates below this point
...
def get_league_tier(*, row: pd.Series) -> int:
    ...
    try:
        ...
        print(msg)
        raise ValueError(
            f"Could not determine league tier for {row['table_title']} in {row['season']}"
        ) from e
...
def get_season(*, row: pd.Series) -> str:
    ...
    # The year gives us two different candidates for the season: {year}-{year+1} and {year-1}-{year}
    # depending on the season start and end dates and the date of the match
    ...
    if not season_1.empty and season_1['start_date'].iloc[0] <= row['match_date'] <= season_1['end_date'].iloc[0]:
        return candidate_season_1
    elif not season_2.empty and season_2['start_date'].iloc[0] <= row['match_date'] <= season_2['end_date'].iloc[0]:
...
def cleanse_data(enfa: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns:
        pd.DataFrame: Cleansed ENFA data.
    ...
    # Work out the season from the match date. This must be called before the league tier is calculated.
    ...
    # Remove unneeded columns - table_title
    enfa = enfa.drop(columns=['table_title'])

    # Make the corrections to the data
    corrections = pd.read_csv("Corrections/enfa_corrections.csv", low_memory=False)
    # Remove known wrong results
    enfa = enfa[~((enfa['match_date'] == '1894-02-03') 
                & (enfa['home_club'] == 'Grimsby Town') & (enfa['away_club'] == 'Middlesbrough Ironopolis'))]
    # Replace with correct data
    enfa = pd.concat([corrections, enfa])

    # Read in missing 2024-2025 season data
    ...
    missing_tiers['match_day_of_week'] = pd.to_datetime(missing_tiers['match_date']).dt.day_name()
    ...
    # Now add the missing tiers to the data
    enfa = pd.concat([enfa, missing_tiers])

    # Set the home_goals and away_goals to integers
    enfa['home_goals'] = enfa['home_goals'].astype(int)
    enfa['away_goals'] = enfa['away_goals'].astype(int)

    # Sort the data
    enfa = enfa.sort_values(by=['season', 'league_tier', 'match_date', 'home_club'])
