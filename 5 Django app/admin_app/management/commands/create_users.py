    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if created:
                ...
                admin_user.save()
                self.stdout.write(
                    self.style.SUCCESS('Admin user "Falcon1234" created successfully')
                )
            else:
                self.stdout.write(
                    self.style.WARNING('Admin user "Falcon1234" already exists')
                )
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS('Regular user "Footsmall" created successfully')
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
            ...
            logger.error(f"Error creating users at line {e.__traceback__.tb_lineno}: {e}")
            self.stdout.write(
